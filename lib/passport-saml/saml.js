...
SAML.prototype.generateServiceProviderMetadata = function( decryptionCert ) {
  ...
  function buildContactPerson(aContacts) {
    ...
    if (aContacts) {
      ...
      aContacts.forEach(function(contact) {
        ...
        //--- Add `<Company>` element
        if (contact.company) {
          contactPerson.Company = contact.company;
        }

        //--- Add `<GivenName>` element
        if (contact.givenName) {
          contactPerson.GivenName = contact.givenName;
        }

        //--- Add `<SurName>` element
        if (contact.surName) {
          contactPerson.SurName = contact.surName;
        }

        //--- Add `<EmailAddress>` elements
        if (contact.emailAddress) {
          if (typeof contact.emailAddress === 'string') {
            contactPerson.EmailAddress = contact.emailAddress;
          } else {
            contact.emailAddress.forEach(function(email){
              (contactPerson.EmailAddress || (contactPerson.EmailAddress = [])).push({ '#text' : email });
            });
          }
        }
        ...
        if (contact.telephoneNumber) {
          if (typeof contact.telephoneNumber === 'string') {
            contactPerson.TelephoneNumber = contact.telephoneNumber;
          } else {
            contact.telephoneNumber.forEach(function(phone){
              (contactPerson.TelephoneNumber || (contactPerson.TelephoneNumber = [])).push({ '#text' : phone });
            });
          }
        }
    ...
    if (organization) {
      ...
      if (organization.displayNames && Array.isArray(organization.displayNames)) {
        organization.displayNames.forEach(function(o) {
          ...
          (Organization.OrganizationDisplayName || (Organization.OrganizationDisplayName = []))
      ...
      if (organization.urls && Array.isArray(organization.urls)) {
        organization.urls.forEach(function(o) {
          ...
          (Organization.OrganizationURL || (Organization.OrganizationURL = []))
  ...
  if (Array.isArray(aContactPerson) && aContactPerson.length) {
    Array.prototype.push.apply((metadata.EntityDescriptor.ContactPerson || (metadata.EntityDescriptor.ContactPerson = [])), aContactPerson);
  }

  //--- Build <Organization> element
  var Organization = buildOrganization(this.options.organization);
  if (Organization) {
    metadata.EntityDescriptor.Organization = Organization;
  }
