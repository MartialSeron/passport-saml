...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  ...
  .then(function(){
    ...
    var declaration = buildXMLDeclaration(self.options);

    callback(null, xmlbuilder.create(request, declaration).end());
  })
  .fail(function(err){
    callback(err);
  })
...
SAML.prototype.generateLogoutRequest = function (req) {
  ...
  var declaration = buildXMLDeclaration(this.options);

  return xmlbuilder.create(request, declaration).end();
};
...
function buildXMLDeclaration(options) {
  return {
    version    : options.version || '1.0',
    encoding   : options.encoding,
    standalone : options.standalone === true ? 'yes' : null,
  };
};

SAML.prototype.generateServiceProviderMetadata = function( decryptionCert ) {
  /**
   * Helper function to build <ContactPerson> elements
   *
   * @todo Handle `<Extensions>` element
   *
   * @param {Object[]} contacts Array of objects representing `<ContactPerson>` elements
   * @param {String} contacts[].type String for `contactType` attribute.
   * The possible values are `technical`, `support`, `administrative`, `billing`, and `other` (default)
   * @param {String} contacts[].company String for `<Company>` element.
   * @param {String} contacts[].givenName String for `<GivenName>` element.
   * @param {String} contacts[].surName String for `<SurName>` element.
   * @param {String|String[]} contacts[].emailAddress String or Array of strings for `<EmailAddress>` elements.
   * @param {String|String[]} contacts[].telephoneNumber String or Array of strings for `<TelephoneNumber>` elements.
   * @returns {Object[]} Return, in an array, an xmlbuilder compatible object for each contact
   */
  function buildContactPerson(aContacts) {
    var types = ['technical', 'support', 'administrative', 'billing', 'other'];
    var aContactPerson = [];
    if (aContacts) {
      if (!Array.isArray(aContacts)) {
        throw new Error('Unable to build ContactPerson element');
      }
      aContacts.forEach(function(contact) {
        ...
        //--- Add valid `contactType` attribute
        if (contact.type && types.indexOf(contact.type) !== -1) {
          contactPerson['@contactType'] = contact.type;
        }

        //--- Add `<Company>` element
        if (contact.company) {
          contactPerson['md:Company'] = contact.company;
        }

        //--- Add `<GivenName>` element
        if (contact.givenName) {
          contactPerson['md:GivenName'] = contact.givenName;
        }

        //--- Add `<SurName>` element
        if (contact.surName) {
          contactPerson['md:SurName'] = contact.surName;
        }

        //--- Add `<EmailAddress>` elements
        if (contact.emailAddress) {
          if (typeof contact.emailAddress === 'string') {
            contactPerson['md:EmailAddress'] = contact.emailAddress;
          } else {
            contact.emailAddress.forEach(function(email){
              (contactPerson['md:EmailAddress'] || (contactPerson['md:EmailAddress'] = [])).push({ '#text' : email });
            });
          }
        }

        //--- Add `<TelephoneNumber>` elements
        if (contact.telephoneNumber) {
          if (typeof contact.telephoneNumber === 'string') {
            contactPerson['md:TelephoneNumber'] = contact.telephoneNumber;
          } else {
            contact.telephoneNumber.forEach(function(phone){
              (contactPerson['md:TelephoneNumber'] || (contactPerson['md:TelephoneNumber'] = [])).push({ '#text' : phone });
            });
          }
        }
        aContactPerson.push(contactPerson);
      });
    }
  ...
  /**
   * Helper function to build <Organisation> element
   *
   * @todo Handle `<Extensions>` element
   *
   * @param {Object} organization Object representing `<Organisation>` element
   * @param {Object[]} organization.name Array of objects representing `<OrganizationName>` elements
   * @param {String} organization.name[].text Text node of `<OrganizationName>` element
   * @param {String} organization.name[].lang Lang attribute of `<OrganizationName>` element
   * @param {Object[]} organization.displayNames Array of objects  representing `<OrganizationDisplayName>` elements
   * @param {String} organization.displayNames[].text Text node of `<OrganizationDisplayName>` element
   * @param {String} organization.displayNames[].lang Lang attribute of `<OrganizationDisplayName>` element
   * @param {Object[]} organization.urls Array of objects representing `<OrganizationURL>` elements
   * @param {String} organization.urls[].text Text node of `<OrganizationURL>` element
   * @param {String} organization.urls[].lang Lang attribute of `<OrganizationURL>` element
   * @returns {undefined|Object} xmlbuilder compatible object representing `<Organisation>` element
   */
  function buildOrganization(organization) {
    if (organization) {
      var Organization = {};
      if (organization.names && Array.isArray(organization.names)) {
        organization.names.forEach(function(o) {
          var OrganizationName = { '#text' : o.text };
          if (o.lang) {
            OrganizationName['@xml:lang'] = o.lang;
          }
          (Organization['md:OrganizationName'] || (Organization['md:OrganizationName'] = []))
            .push(OrganizationName);
        });
      }
      if (organization.displayNames && Array.isArray(organization.displayNames)) {
        organization.displayNames.forEach(function(o) {
          var OrganizationDisplayName = { '#text' : o.text };
          if (o.lang) {
            OrganizationDisplayName['@xml:lang'] = o.lang;
          }
          (Organization['md:OrganizationDisplayName'] || (Organization['md:OrganizationDisplayName'] = []))
            .push(OrganizationDisplayName);
        });
      }
      if (organization.urls && Array.isArray(organization.urls)) {
        organization.urls.forEach(function(o) {
          var OrganizationURL = { '#text' : o.text };
          if (o.lang) {
            OrganizationURL['@xml:lang'] = o.lang;
          }
          (Organization['md:OrganizationURL'] || (Organization['md:OrganizationURL'] = []))
  ...
  if (this.options.decryptionPvk) {
    ...
    var keyDescriptor = {
      ...
      'md:EncryptionMethod' : encryptionMethods,
    };

    metadata['md:EntityDescriptor']['md:SPSSODescriptor']['md:KeyDescriptor'] = keyDescriptor;

    //--- Possibility to separate <md:KeyDescriptor @use="signing" /> and <md:KeyDescriptor @use="encryption" />
    if (this.options.separateSigningAndEncryption === true) {
      var keyDescriptorSign = JSON.parse(JSON.stringify(keyDescriptor));
      var keyDescriptorEncr = JSON.parse(JSON.stringify(keyDescriptor));
      keyDescriptorSign['@use'] = 'signing';
      keyDescriptorEncr['@use'] = 'encryption';
  ...
  //--- Build <ContactPerson> elements
  var aContactPerson = buildContactPerson(this.options.contactPerson);
  if (Array.isArray(aContactPerson) && aContactPerson.length) {
    Array.prototype.push.apply((metadata['md:EntityDescriptor']['md:ContactPerson'] || (metadata['md:EntityDescriptor']['md:ContactPerson'] = [])), aContactPerson);
  }

  //--- Build <Organization> element
  var Organization = buildOrganization(this.options.organization);
  if (Organization) {
    metadata['md:EntityDescriptor']['md:Organization'] = Organization;
  }

  var declaration = buildXMLDeclaration(this.options);

  return xmlbuilder.create(metadata, declaration).end({ pretty: true, indent: '  ', newline: '\n' });
};
